...
namespace SingularityGroup.HotReload
{
	internal class RegisterPatchesResult
	{
		...
		public List<SField> addedFields = new();
		...
		public readonly List<string> patchExceptions = new();
	}
	...
	internal class FieldHandler
	{
		...
		public FieldHandler(Func<Type, FieldInfo, bool> storeField, Func<Type, string, bool> hideField,
		                    Action<Type, FieldInfo, FieldInfo> registerInspectorFieldAttributes)
		{
	...
	internal class CodePatcher
	{
		...
		readonly HashSet<string> seenResponses = new();
		...
		CodePatcher()
		{
			...
			if (!UnityHelper.IsEditor)
			{
				...
				{
					Log.Error($"{Translations.Logging.LoadingPatchesFromDiskError}\n{ex}");
				}
			}
#if UNITY_EDITOR
		...
		{
			PlayerLog(Translations.Logging.LoadingPatchesFromFile, filePath);
			var file = new FileInfo(filePath);
			if (file.Exists)
			{
				var bytes = File.ReadAllText(filePath);
				var patches = JsonConvert.DeserializeObject<List<MethodPatchResponse>>(bytes);
				PlayerLog(Translations.Logging.LoadedPatchesFromDisk, patches.Count.ToString());
				foreach (var patch in patches)
				{
					RegisterPatches(patch, false);
				}
			}
		}
		...
		{
			PlayerLog(Translations.Logging.RegisterPatches, string.Join("\n", patches.failures),
		...
		{
			PlayerLog(Translations.Logging.ApplyPatchesPending, pendingPatches.Count);
			EnsureSymbolResolver();
			...
			{
				Log.Warning($"{Translations.Logging.ExceptionHandlingMethodPatch}\n{ex}");
			}
			finally
			{
		...
		static void WarnOnSceneLoad()
		{
			SceneManager.sceneLoaded += (_, __) =>
			{
				...
				Log.Warning(Translations.Logging.SceneLoadedWithNewUnityEventMethods);
				didLog = true;
			};
		}

		static HashSet<MethodBase> unityEventMethods = new();
		...
		static bool EnsureUnityEventMethod(MethodBase newMethod)
		{
			...
			{
				Log.Warning(Translations.Logging.ExceptionEnsureUnityEventMethod, ex.GetType().Name, ex.Message);
				return false;
			}
		}
			...
			foreach (var patch in response.patches)
			{
				try
				{
					foreach (var sMethod in patch.newMethods)
					{
						...
						if (isUnityEvent) unityEventMethods.Add(newMethod);
					...
					for (var i = 0; i < patch.modifiedMethods.Length; i++)
					{
						...
						var err = PatchMethod(response.id, sOriginalMethod, sPatchMethod, patch.unityJobs.Length > 0, result);
				...
				{
					RequestHelper.RequestEditorEventWithRetry(new Stat(StatSource.Client, StatLevel.Error, StatFeature.Patching, StatEventType.Exception),
						new EditorExtraData
						{
							{StatKey.PatchId, patch.patchId},
							{StatKey.Detailed_Exception, ex.ToString()}
						}).Forget();
					result.patchExceptions.Add($"{Translations.Logging.ExceptionApplyingPatch}\nException: {ex}");
				}
			}
		}
		...
		void HandleRemovedUnityMethods(SMethod[] removedMethods)
		{
			...
			foreach (var sMethod in removedMethods)
			{
				...
				{
					Log.Warning(Translations.Logging.ExceptionRemoveUnityEventMethod, ex.GetType().Name, ex.Message);
				}
			}
		}
		...
		void RegisterNewFieldInitializers(MethodPatchResponse resp)
		{
			for (var i = 0; i < resp.addedFieldInitializerFields.Length; i++)
			{
				...
				try
				{
					...
					{
						Log.Warning(Translations.Logging.FailedRegisteringInitializerInvalidMethod, sField.fieldName, sField.declaringType.typeName);
						continue;
					}
				...
				catch (Exception e)
				{
					RequestHelper.RequestEditorEventWithRetry(
						new Stat(StatSource.Client, StatLevel.Error, StatFeature.Patching, StatEventType.RegisterFieldInitializer), new EditorExtraData
						{
							{StatKey.PatchId, resp.id},
							{StatKey.Detailed_Exception, e.ToString()}
						}).Forget();
					Log.Warning(Translations.Logging.FailedRegisteringInitializerException, sField.fieldName, sField.declaringType.typeName, e.Message);
				}
			}
		}
		...
		void RegisterNewFieldDefinitions(MethodPatchResponse resp)
		{
			foreach (var sField in resp.newFieldDefinitions)
			{
				...
				{
					RequestHelper.RequestEditorEventWithRetry(
						new Stat(StatSource.Client, StatLevel.Error, StatFeature.Patching, StatEventType.RegisterFieldDefinition), new EditorExtraData
						{
							{StatKey.PatchId, resp.id},
							{StatKey.Detailed_Exception, e.ToString()}
						}).Forget();
					Log.Warning(Translations.Logging.FailedRegisteringNewFieldDefinitions, sField.fieldName, sField.declaringType.typeName, e.Message);
				}
			}
		}
		...
		void RemoveOldFieldInitializers(MethodPatchResponse resp)
		{
			foreach (var sField in resp.removedFieldInitializers)
			{
				...
				{
					RequestHelper.RequestEditorEventWithRetry(
						new Stat(StatSource.Client, StatLevel.Error, StatFeature.Patching, StatEventType.UnregisterFieldInitializer), new EditorExtraData
						{
							{StatKey.PatchId, resp.id},
							{StatKey.Detailed_Exception, e.ToString()}
						}).Forget();
					Log.Warning(Translations.Logging.FailedRemovingInitializer, sField.fieldName, sField.declaringType.typeName, e.Message);
				}
			}
		}
		...
		internal void HandleReshapedFields(MethodPatchResponse resp)
		{
			foreach (var patch in resp.patches)
			{
				...
				foreach (var f in removedReshapedFields)
				{
					...
					{
						RequestHelper.RequestEditorEventWithRetry(
							new Stat(StatSource.Client, StatLevel.Error, StatFeature.Patching, StatEventType.ClearHolders), new EditorExtraData
							{
								{StatKey.PatchId, resp.id},
								{StatKey.Detailed_Exception, e.ToString()}
							}).Forget();
						Log.Warning(Translations.Logging.FailedRemovingFieldValue, f.fieldName, f.declaringType.typeName, e.Message);
					}
				}
				...
				for (var i = 0; i < renamedReshapedFieldsFrom.Length; i++)
				{
					...
					try
					{
						...
						if (!AreSTypesCompatible(fromField.declaringType, toField.declaringType) || fieldType != toFieldType ||
					...
					{
						RequestHelper.RequestEditorEventWithRetry(new Stat(StatSource.Client, StatLevel.Error, StatFeature.Patching, StatEventType.MoveHolders),
							new EditorExtraData
							{
								{StatKey.PatchId, resp.id},
								{StatKey.Detailed_Exception, e.ToString()}
							}).Forget();
						Log.Warning(Translations.Logging.FailedMovingFieldValue, fromField, toField, toField.declaringType.typeName, e.Message);
					}
				}
			}
		}
		...
		{
			if (one.isGenericParameter != two.isGenericParameter) return false;
			if (one.metadataToken != two.metadataToken) return false;
			if (one.assemblyName != two.assemblyName) return false;
			if (one.genericParameterPosition != two.genericParameterPosition) return false;
			if (one.typeName != two.typeName) return false;
			return true;
		}
			...
			foreach (var patch in resp.patches)
			{
				...
				for (var i = 0; i < propertyAttributesFieldOriginal.Length; i++)
				{
					...
					catch (Exception e)
					{
						...
						Log.Warning(Translations.Logging.FailedUpdatingFieldAttributes, original.fieldName, original.declaringType.typeName, e.Message);
					}
				}
			}
		}
		...
		internal void HandleNewFields(string patchId, RegisterPatchesResult result, SField[] sFields)
		{
			...
			{
				if (!sField.serializable) continue;
				try
				{
					var declaringType = SymbolResolver.Resolve(sField.declaringType);
				...
				{
					RequestHelper.RequestEditorEventWithRetry(
						new Stat(StatSource.Client, StatLevel.Error, StatFeature.Patching, StatEventType.AddInspectorField), new EditorExtraData
						{
							{StatKey.PatchId, patchId},
							{StatKey.Detailed_Exception, e.ToString()}
						}).Forget();
					Log.Warning(Translations.Logging.FailedAddingFieldToInspector, sField.fieldName, sField.declaringType.typeName, e.Message);
				}
			}
		...
		{
			if (alteredFields == null) return;
			var alteredFieldHidden = false;
			foreach (var sField in alteredFields)
			{
				try
				{
					var declaringType = SymbolResolver.Resolve(sField.declaringType);
					if (fieldHandler?.hideField?.Invoke(declaringType, sField.fieldName) == true) alteredFieldHidden = true;
				}
				catch (Exception e)
				{
					RequestHelper.RequestEditorEventWithRetry(
						new Stat(StatSource.Client, StatLevel.Error, StatFeature.Patching, StatEventType.HideInspectorField), new EditorExtraData
						{
							{StatKey.PatchId, patchId},
							{StatKey.Detailed_Exception, e.ToString()}
						}).Forget();
					Log.Warning(Translations.Logging.FailedHidingFieldFromInspector, sField.fieldName, sField.declaringType.typeName, e.Message);
				}
			}

			if (alteredFieldHidden) result.inspectorModified = true;
		}
#endif

		Dictionary<MethodBase, MethodBase> previousPatchMethods = new();
		public IEnumerable<MethodBase> OriginalPatchMethods => previousPatchMethods.Keys;
		List<MethodBase> newMethods = new();
		...
		string PatchMethod(string patchId, SMethod sOriginalMethod, SMethod sPatchMethod, bool containsBurstJobs, RegisterPatchesResult patchesResult)
		{
			try
			{
				...
				{
					RequestHelper.RequestEditorEventWithRetry(
						new Stat(StatSource.Client, StatLevel.Error, StatFeature.Patching, StatEventType.DebuggerAttached), new EditorExtraData
						{
							{StatKey.PatchId, patchId}
						}).Forget();
					return Translations.Logging.DebuggerAttachedNotAllowed;
				}

				if (DateTime.UtcNow - start > TimeSpan.FromMilliseconds(500))
					Log.Info(Translations.Logging.HotReloadApplyTook, (DateTime.UtcNow - start).TotalMilliseconds);

				if (state.match == null)
				{
					RequestHelper.RequestEditorEventWithRetry(new Stat(StatSource.Client, StatLevel.Error, StatFeature.Patching, StatEventType.MethodMismatch),
						new EditorExtraData
						{
							{StatKey.PatchId, patchId}
						}).Forget();
					return string.Format(Translations.Logging.MethodMismatch, sOriginalMethod.simpleName, patchMethod.Name);
				}

				PlayerLog(Translations.Logging.DetourMethod, sOriginalMethod.metadataToken, patchMethod.Name, state.offset);
				DetourResult result;
				...
				if (result.success)
				{
					...
					if (!previousPatchMethods.TryGetValue(state.match, out previousMethod)) previousMethod = state.match;
				...
				RequestHelper.RequestEditorEventWithRetry(new Stat(StatSource.Client, StatLevel.Error, StatFeature.Patching, StatEventType.Failure),
					new EditorExtraData
					{
						{StatKey.PatchId, patchId},
		...
		struct ResolveMethodState
		{
			...
			public ResolveMethodState With(bool? tryLowerTokens = null, bool? tryHigherTokens = null, MethodBase match = null, int? offset = null) =>
				new(originalMethod, offset ?? this.offset, tryLowerTokens ?? this.tryLowerTokens, tryHigherTokens ?? this.tryHigherTokens, match ?? this.match);
		}
		...
		MethodBase TryGetMethodBaseWithRelativeToken(SMethod sOriginalMethod, int offset) =>
			symbolResolver.Resolve(new SMethod(sOriginalMethod.assemblyName, sOriginalMethod.displayName, sOriginalMethod.metadataToken + offset,
				sOriginalMethod.simpleName));

		string HandleMethodPatchFailure(SMethod method, Exception exception) => string.Format(Translations.Logging.FailedToApplyPatchForMethod,
			method.displayName, method.assemblyName, exception);
		...
		void EnsureSymbolResolver()
		{
			if (symbolResolver == null)
			{
				...
				for (var i = 0; i < assemblies.Length; i++)
				{
					...
					List<Assembly> list;
					if (!assembliesByName.TryGetValue(name, out list)) assembliesByName.Add(name, list = new List<Assembly>());
		...
		async Task SaveAppliedPatchesNoLock(string filePath)
		{
			...
			if (string.IsNullOrEmpty(dir)) throw new ArgumentException(string.Format(Translations.Logging.InvalidPath, filePath), nameof(filePath));
			Directory.CreateDirectory(dir);
			var history = patchHistory.ToList();

			PlayerLog(Translations.Logging.SavingAppliedPatches, history.Count, filePath);
		...
		class SimpleMethodComparer : IEqualityComparer<SMethod>
		{
			...
			public int GetHashCode(SMethod x) => x.metadataToken;
		}
	}
}