...
namespace SingularityGroup.HotReload
{
	internal class ServerHandshake
	{
		...
		TaskCompletionSource<Result> firstHandshake = new();
		...
		async Task<bool> VerifyResults(Result results, PatchServerInfo server)
		{
			...
			if (results.HasFlag(Result.DifferentProject))
			{
				...
				{
					summary = Translations.Dialogs.DifferentProjectSummary,
					suggestion = Translations.Dialogs.DifferentProjectSuggestion,
			...
			if (results.HasFlag(Result.DifferentCommit))
			{
				...
				{
					summary = Translations.Dialogs.DifferentCommitSummary,
					suggestion = Translations.Dialogs.DifferentCommitSuggestion,
		...
		static async Task<Result> RequestHandshake(PatchServerInfo info)
		{
			...
			Log.Info(Translations.Logging.RequestHandshakeToServer, info.hostName);
			//Log.Debug("Handshake with projectOmissionRegex: \"{0}\"", buildInfo.projectOmissionRegex);
			var response = await RequestHelper.RequestHandshake(info, buildInfo.DefineSymbolsAsHashSet, buildInfo.projectOmissionRegex);
			...
			if (remoteBuildTarget == null)
			{
				...
				Log.Warning(Translations.Errors.HandshakeFailedInvalidBuildTarget, buildInfo.activeBuildTarget);
				results |= Result.QuietWarning;
			}
			else if (remoteBuildTarget != buildInfo.activeBuildTarget)
			{
				Log.Warning(Translations.Errors.BuildTargetMismatch, remoteBuildTarget, buildInfo.activeBuildTarget);
				results |= Result.QuietWarning;
			}
