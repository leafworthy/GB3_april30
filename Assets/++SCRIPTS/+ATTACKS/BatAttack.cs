...
public class BatAttack : Attacks
{
	...
	private void Init()
	{
		...
		if (owner != null) owner.Controller.Attack1RightTrigger.OnPress += Player_AttackPress;
	...
	private void RegularAttackHit(int attackType)
	{
		...
		foreach (var col in closest2)
		{
			...
			HitTarget(GetAttackDamage(attackType), _life, .7f);
