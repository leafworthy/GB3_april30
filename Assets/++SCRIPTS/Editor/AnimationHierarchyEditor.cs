...
public class AnimationHierarchyEditor : EditorWindow {
	...
	[MenuItem("Window/AnimatorComponent Hierarchy Editor")]
	...
	void OnGUI() {
		...
		if (animationClips.Count > 0 ) {
			...
			EditorGUILayout.BeginHorizontal();
			GUILayout.Label("AnimatorComponent Clip:", GUILayout.Width(columnWidth));
			...
			GUILayout.Label("Please select an AnimatorComponent Clip");
		}
	}
	...
	void ReplaceRoot(string oldRoot, string newRoot)
	{
		...
		for ( int iCurrentClip = 0; iCurrentClip < animationClips.Count; iCurrentClip++ )
		{
			...
			Undo.RecordObject(animationClip, "AnimatorComponent Hierarchy Root Change");
			...
			for ( int iCurrentPath = 0; iCurrentPath < pathsKeys.Count; iCurrentPath ++)
			{
				...
				EditorUtility.DisplayProgressBar(
					"AnimatorComponent Hierarchy Progress", 
	...
	void UpdatePath(string oldPath, string newPath) 
	{
		...
		{
			AnimationClip animationClip =  animationClips[iCurrentClip];
			Undo.RecordObject(animationClip, "AnimatorComponent Hierarchy Change");
