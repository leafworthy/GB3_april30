...
using __SCRIPTS.Plugins.AstarPathfindingProject.PackageTools;
using UnityEngine;

namespace __SCRIPTS.Plugins.AstarPathfindingProject.Utilities {
	/// <summary>
	...
	public class ProceduralGridMoveAbility : VersionedMonoBehaviour {
		/// <summary>
		...
		void Start () {
			...
			if (graph == null) {
				...
				if (graphIndex >= AstarPath.active.data.graphs.Length) throw new System.Exception("The ProceduralGridMoveAbility was configured to use graph index " + graphIndex + ", but only " + AstarPath.active.data.graphs.Length + " graphs exist");

				graph = AstarPath.active.data.graphs[graphIndex] as GridGraph;
				if (graph == null) throw new System.Exception("The ProceduralGridMoveAbility was configured to use graph index " + graphIndex + " but that graph either does not exist or is not a GridGraph or LayerGridGraph");
			}
