Shader "Light2D/Sprites/LitAbove"
{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed3 OutputColor(fixed4 spritePixel, v2f IN) {
				fixed3 lightPixel = SL2D_Light(IN.worldPos);

                float delta = _NormalSize;

                fixed lightPixelLeft = SL2D_Light(IN.worldPos + float2(-delta, 0));
                fixed lightPixelRight = SL2D_Light(IN.worldPos + float2(delta, 0));

                fixed lightPixelUp = SL2D_Light(IN.worldPos + float2(0, delta));
                fixed lightPixelDown = SL2D_Light(IN.worldPos + float2(0, -delta));
