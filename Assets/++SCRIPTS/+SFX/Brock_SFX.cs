...
public class Brock_SFX : MonoBehaviour
{
	...
	private void OnEnable()
	{
		...
		chargeAttack.OnChargeStop += OnChargeStop;
		kunaiAttack.OnThrow += KunaiAttackOnThrow;
	}

	
	
	private void OnChargeStop()
	{
	...
	private void KunaiAttackOnThrow(Vector3 vector3, Vector3 vector4, float arg3, Life arg4, bool arg5) =>
		SFX.sounds.brock_bat_swing_sounds.PlayRandomAt(transform.position);

	private void ChargeAttack_OnSpecialAttackHit() => SFX.sounds.brock_homerunhit_sounds.PlayRandomAt(transform.position);
	private void ChargeAttack_OnChargePress() => SFX.I.StartOngoingSound();
