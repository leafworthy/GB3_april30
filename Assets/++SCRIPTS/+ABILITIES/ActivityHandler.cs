...
namespace __SCRIPTS
{
	...
	public class ActivityHandler
	{
		...
		public void StopSafely(GangstaBean.Core.IActivity activity)
		{
			if (currentActivity?.VerbName != activity.VerbName)
			{
		...
		public bool CompleteCurrentActivity(CompletionReason reason, GangstaBean.Core.IActivity newActivity = null)
		{
			...
			// Check for new interface method first
			if (currentActivity.TryCompleteGracefully(reason, newActivity))
			{
				handled = true;
			}
			// Fall back to legacy interface for backward compatibility
			else if (currentActivity is ICompletableActivity completableActivity)
			{
				handled = completableActivity.CompleteActivity(reason, newActivity);
			}
