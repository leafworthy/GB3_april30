using RisingTextNamespace;
using UnityEngine;

public class DEBREE : Singleton<DEBREE>
{
	private void Start()
	{
	...
	private void CreateDamageRisingText(Attack attack)
	{
		...
		var risingText = Maker.Make(ASSETS.FX.risingTextPrefab, attack.DestinationWithHeight);
	...
	{
		var hitList = ASSETS.FX.GetBulletHits(attack.DestinationLife.DebreeType);
	...
	private void MakeDebree(Attack attack, Life hitLife)
	{
		...
		{
			//----->
			var forwardDebree = Maker.Make(ASSETS.FX.GetDebree(attack.DestinationLife.DebreeType), attack.DestinationFloorPoint);
		...
		Maker.Unmake(forwardDebree, 3);
			...
			var backwardDebree = Maker.Make(ASSETS.FX.GetDebree(attack.DestinationLife.DebreeType), attack.DestinationFloorPoint);
	...
	{
		var blood = Maker.Make(ASSETS.FX.bloodspray.GetRandom(), attack.DestinationFloorPoint);
