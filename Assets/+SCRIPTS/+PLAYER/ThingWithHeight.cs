...
public class ThingWithHeight : ServiceUser
{
	...
	public void UpdateDistanceToCurrentLandable()
	{
		...
		{
			isOverLandable = true;
			if (canLand) AddRenderersToCurrentLandable();
		}
		else
		{
	...
	private void RemoveRenderersFromCurrentLandable()
	{
		...
		foreach (var renderer in Sorting.renderersToSort.Where(renderer =>
			         currentLandableSorting.renderersToSort.Contains(renderer)))
		{
