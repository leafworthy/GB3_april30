...
namespace __SCRIPTS.Plugins._ISOSORT
{
	...
	{
		private static readonly Dictionary<int, bool> circularDepData = new();
		private static readonly List<IsoSpriteSorting> circularDepStack = new(64);

		private static readonly HashSet<int> visited = new();
		private static readonly List<IsoSpriteSorting> allSprites = new(64);

		public static void Sort(List<IsoSpriteSorting> staticSprites, List<IsoSpriteSorting> movableSprites, List<IsoSpriteSorting> sorted)
		{
			allSprites.Clear();
		...
		{
			if (_circularReferenceStack.Count <= 1) return;
			...
			for (var i = _circularReferenceStack.Count - 2; i >= 0; i--)
			{
				...
				if (sorter != startingSorter) continue;
				repeatIndex = i;
				break;
			}
