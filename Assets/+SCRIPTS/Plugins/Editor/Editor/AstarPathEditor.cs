...
using __SCRIPTS.Plugins.Editor.Editor.GraphEditors;
using UnityEditor;
using UnityEngine;
		...
		void DeserializeGraphs (byte[] bytes) {
			try {
				...
				for (int i = 0; i < graphEditors.Length; i++) {
					...
					if (data != null) TinyJsonDeserializer.Deserialize(data, graphEditors[i].GetType(), graphEditors[i], script.gameObject);
				}
			} catch (System.Exception e) {
