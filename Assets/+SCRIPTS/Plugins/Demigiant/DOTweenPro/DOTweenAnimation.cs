...
namespace DG.Tweening
{
	...
	public class DOTweenAnimation : ABSAnimationComponent
	{
		...
		readonly List<Tween> _tmpTweens = new();
		...
		public void CreateTween(bool regenerateIfExists = false, bool andPlay = true)
		{
			...
			if (target == null || tweenGO == null)
			{
				...
				else
				{
					...
					Debug.LogWarning(
						string.Format("{0} :: This DOTweenAnimation's target/GameObject is unset: the tween will not be created.", gameObject.name),
						gameObject);
				}
			...
			switch (animationType)
			{
				...
				case AnimationType.Move:
					if (useTargetAsV3)
					{
						...
						{
							Debug.LogWarning(string.Format("{0} :: This tween's TO target is NULL, a Vector3 of (0,0,0) will be used instead", gameObject.name),
								gameObject);
						...
						else
						{
							...
							if (targetType == TargetType.RectTransform)
							{
								...
								{
									Debug.LogWarning(
										string.Format("{0} :: This tween's TO target should be a RectTransform, a Vector3 of (0,0,0) will be used instead",
											gameObject.name), gameObject);
									endValueV3 = Vector3.zero;
								}
								else
								{
									...
									if (rTarget == null)
										Debug.LogWarning(
											string.Format("{0} :: This tween's target and TO target are not of the same type. Please reassign the values",
												gameObject.name), gameObject);
									else
									{
			...
			tween.SetTarget(setTarget).SetDelay(delay).SetLoops(loops, loopType).SetAutoKill(autoKill).OnKill(() => tween = null);
		...
		/// <summary>
		/// Pauses/unpauses (depending on the current weaponState) all tweens whose target-id is the same as the one set by this animation
		/// </summary>
		...
		/// </summary>
		/// <returns></returns>
		GameObject GetTweenGO() => targetIsSelf ? gameObject : targetGO;

		/// <summary>
		/// Returns the GameObject which should be used/retrieved for WithDestinationLife
		/// </summary>
		GameObject GetTweenTarget() => targetIsSelf || !tweenTargetIsTargetGO ? gameObject : targetGO;
		...
		void ReEvaluateRelativeTween()
		{
			...
			if (tweenGO == null)
			{
				Debug.LogWarning(string.Format("{0} :: This DOTweenAnimation's target/GameObject is unset: the tween will not be created.", gameObject.name),
					gameObject);
				return;
			}
			...
			else if (animationType == AnimationType.LocalMove) ((Tweener) tween).ChangeEndValue(tweenGO.transform.localPosition + endValueV3, true);
		}
	...
	public static class DOTweenAnimationExtensions
	{
		...
		public static bool IsSameOrSubclassOf<T>(this Component t) => t is T;
	}
}