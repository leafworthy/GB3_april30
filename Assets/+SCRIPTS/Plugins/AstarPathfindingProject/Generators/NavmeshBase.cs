...
using __SCRIPTS.Plugins.AstarPathfindingProject.Core.Nodes;
using __SCRIPTS.Plugins.AstarPathfindingProject.Core.Serialization;
using __SCRIPTS.Plugins.AstarPathfindingProject.Generators.NodeClasses;
using __SCRIPTS.Plugins.AstarPathfindingProject.Generators.Utilities;
using __SCRIPTS.Plugins.AstarPathfindingProject.Navmesh;
using __SCRIPTS.Plugins.AstarPathfindingProject.Utilities;
using UnityEngine;
using UnityEngine.Profiling;

namespace __SCRIPTS.Plugins.AstarPathfindingProject.Generators
{
	...
	public abstract class NavmeshBase : NavGraph, INavmesh, INavmeshHolder, ITransformedGraph
	{
		...
		[JsonMember] public Vector3 forcedBoundsSize = new(100, 40, 100);
		...
		/// </code>
		/// </summary>
		public NavmeshTile GetTile(int x, int z) => tiles[x + z * tileXCount];
		...
		public static int GetTileIndex(int index) => index >> TileIndexOffset & TileIndexMask;

		public int GetVertexArrayIndex(int index) => index & VertexIndexMask;
		...
		/// </summary>
		public NavmeshTile[] GetTiles() => tiles;

		/// <summary>
		/// Returns a bounds object with the bounding box of a group of tiles.
		/// The bounding box is defined in world space.
		/// </summary>
		public Bounds GetTileBounds(IntRect rect) => GetTileBounds(rect.xmin, rect.ymin, rect.Width, rect.Height);

		/// <summary>
		/// Returns a bounds object with the bounding box of a group of tiles.
		/// The bounding box is defined in world space.
		/// </summary>
		public Bounds GetTileBounds(int x, int z, int width = 1, int depth = 1) => transform.Transform(GetTileBoundsInGraphSpace(x, z, width, depth));

		public Bounds GetTileBoundsInGraphSpace(IntRect rect) => GetTileBoundsInGraphSpace(rect.xmin, rect.ymin, rect.Width, rect.Height);
		...
		public Bounds GetTileBoundsInGraphSpace(int x, int z, int width = 1, int depth = 1)
		{
			...
			b.SetMinMax(new Vector3(x * TileWorldSizeX, 0, z * TileWorldSizeZ),
			...
			if (tiles != null)
			{
				...
				for (var tileIndex = 0; tileIndex < tiles.Length; tileIndex++)
				{
					...
					if (tile != null)
					{
						...
						for (var nodeIndex = 0; nodeIndex < tile.nodes.Length; nodeIndex++) tile.nodes[nodeIndex].UpdatePositionFromVertices();
		...
		public IntRect GetTouchingTiles(Bounds bounds, float margin = 0)
		{
			...
			var r = new IntRect(Mathf.FloorToInt((bounds.min.x - margin) / TileWorldSizeX), Mathf.FloorToInt((bounds.min.z - margin) / TileWorldSizeZ),
				Mathf.FloorToInt((bounds.max.x + margin) / TileWorldSizeX), Mathf.FloorToInt((bounds.max.z + margin) / TileWorldSizeZ));
			// Clamp to bounds
		...
		{
			// Calculate world bounds of all affected tiles
			var r = new IntRect(Mathf.FloorToInt(rect.xMin / TileWorldSizeX), Mathf.FloorToInt(rect.yMin / TileWorldSizeZ),
				Mathf.FloorToInt(rect.xMax / TileWorldSizeX), Mathf.FloorToInt(rect.yMax / TileWorldSizeZ));
		...
		public IntRect GetTouchingTilesRound(Bounds bounds)
		{
			...
			//Calculate world bounds of all affected tiles
			var r = new IntRect(Mathf.RoundToInt(bounds.min.x / TileWorldSizeX), Mathf.RoundToInt(bounds.min.z / TileWorldSizeZ),
				Mathf.RoundToInt(bounds.max.x / TileWorldSizeX) - 1, Mathf.RoundToInt(bounds.max.z / TileWorldSizeZ) - 1);
			//Clamp to bounds
			r = IntRect.Intersection(r, new IntRect(0, 0, tileXCount - 1, tileZCount - 1));
			return r;
		}
		...
		protected void ConnectTileWithNeighbours(NavmeshTile tile, bool onlyUnflagged = false)
		{
			...
			for (var zo = -1; zo <= 1; zo++)
			{
				...
				for (var xo = -1; xo <= 1; xo++)
				{
					...
					if (!onlyUnflagged || !otherTile.flag) ConnectTiles(otherTile, tile);
				}
			}
		}
		...
		public override NNInfoInternal GetNearest(Vector3 position, NNConstraint constraint, GraphNode hint) =>
			GetNearestForce(position, constraint != null && constraint.distanceXZ ? NNConstraintDistanceXZ : null);
		...
		protected void FillWithEmptyTiles()
		{
			...
			{
				for (var x = 0; x < tileXCount; x++) tiles[z * tileXCount + x] = NewEmptyTile(x, z);
			}
		}
		...
		protected static void CreateNodeConnections(TriangleMeshNode[] nodes)
		{
			...
			for (var i = 0; i < nodes.Length; i++)
			{
				...
				for (var a = 0; a < av; a++)
				{
					...
					if (!nodeRefs.ContainsKey(key)) nodeRefs.Add(key, i);
				}
			}
				...
				for (var a = 0; a < av; a++)
				{
					...
					if (nodeRefs.TryGetValue(new Int2(second, first), out connNode))
					{
						...
						for (var b = 0; b < bv; b++)
						{
							...
							{
								connections.Add(new Connection(other, (uint) (node.position - other.position).costMagnitude, (byte) a));
								break;
							}
						}
					}
								...
								if (Math.Abs(bVertex1[coord] - midpoint) < 2 && Math.Abs(bVertex2[coord] - midpoint) < 2)
								{
									...
									if (maxalt > minalt2 && minalt < maxalt2)
									{
										...
										if (aVertex1 == bVertex1 && aVertex2 == bVertex2 || aVertex1 == bVertex2 && aVertex2 == bVertex1 ||
										    VectorMath.SqrDistanceSegmentSegment((Vector3) aVertex1, (Vector3) aVertex2, (Vector3) bVertex1,
											    (Vector3) bVertex2) < MaxTileConnectionEdgeDistance * MaxTileConnectionEdgeDistance)
										{
		...
		{
			for (var i = 0; i < batchNodesToDestroy.Count; i++) batchNodesToDestroy[i].TemporaryFlag1 = true;
			...
			for (var i = 0; i < batchNodesToDestroy.Count; i++)
			{
				...
				for (var j = 0; j < node.connections.Length; j++)
				{
					...
					if (!neighbour.TemporaryFlag1) neighbour.RemoveConnection(node);
				}
		...
		public void ReplaceTile(int x, int z, Int3[] verts, int[] tris)
		{
			...
			if (x + w > tileXCount || z + d > tileZCount || x < 0 || z < 0)
				throw new System.ArgumentException("Tile is placed at an out of bounds position or extends out of the graph bounds (" + x + ", " + z + " [" +
			...
			if (!Mathf.Approximately(x * TileWorldSizeX * Int3.FloatPrecision, (float) Math.Round(x * TileWorldSizeX * Int3.FloatPrecision)))
				Debug.LogWarning("Possible numerical imprecision. Consider adjusting tileSize and/or cellSize");
			if (!Mathf.Approximately(z * TileWorldSizeZ * Int3.FloatPrecision, (float) Math.Round(z * TileWorldSizeZ * Int3.FloatPrecision)))
				Debug.LogWarning("Possible numerical imprecision. Consider adjusting tileSize and/or cellSize");
		...
		protected void CreateNodes(TriangleMeshNode[] buffer, int[] tris, int tileIndex, uint graphIndex)
		{
			...
			for (var i = 0; i < buffer.Length; i++)
			{
				...
				if (RecalculateNormals &&
				    !VectorMath.IsClockwiseXZ(node.GetVertexInGraphSpace(0), node.GetVertexInGraphSpace(1), node.GetVertexInGraphSpace(2)))
				{
		...
		public NavmeshBase() => navmeshUpdateData = new NavmeshUpdates.NavmeshUpdateSettings(this);
		...
		public override void OnDrawGizmos(RetainedGizmos gizmos, bool drawNodes)
		{
			...
			if (tiles != null && (showMeshSurface || showMeshOutline || showNodeConnections))
			{
				...
				for (var i = 0; i < tiles.Length; i++)
				{
					...
					var nodes = tiles[i].nodes;
					for (var j = 0; j < nodes.Length; j++) hasher.HashNode(nodes[j]);
							...
							using (var helper = gizmos.GetGizmoHelper(active, hasher))
							{
								...
								if (showNodeConnections)
								{
									for (var ti = startTileIndex; ti <= i; ti++)
									{
										...
										for (var j = 0; j < tileNodes.Length; j++) helper.DrawConnections(tileNodes[j]);
									}
								}
							}
						}
		...
		protected override void SerializeExtraInfo(GraphSerializationContext ctx)
		{
			...
			for (var z = 0; z < tileZCount; z++)
			{
				for (var x = 0; x < tileXCount; x++)
				{
					...
					for (var i = 0; i < tile.verts.Length; i++) ctx.SerializeInt3(tile.verts[i]);

					writer.Write(tile.vertsInGraphSpace.Length);
					for (var i = 0; i < tile.vertsInGraphSpace.Length; i++) ctx.SerializeInt3(tile.vertsInGraphSpace[i]);

					writer.Write(tile.nodes.Length);
					for (var i = 0; i < tile.nodes.Length; i++) tile.nodes[i].SerializeNode(ctx);
				}
			}
		}
		...
		protected override void DeserializeExtraInfo(GraphSerializationContext ctx)
		{
			...
			for (var z = 0; z < tileZCount; z++)
			{
				for (var x = 0; x < tileXCount; x++)
				{
					...
					for (var i = 0; i < tile.verts.Length; i++) tile.verts[i] = ctx.DeserializeInt3();
					...
					if (ctx.meta.version.Major >= 4)
					{
						...
						for (var i = 0; i < tile.verts.Length; i++) tile.vertsInGraphSpace[i] = ctx.DeserializeInt3();
					}
					else
					{
						// Compatibility
