...
namespace __SCRIPTS
{
	public class DayNightCycle : Singleton<DayNightCycle>
	{
		...
		[Tooltip("How fast time passes")] public float timeMultiplier = 1.0f;
		[Tooltip("Pause the cycle")] public bool pauseCycle;

		[Header("Light Intensity"), Tooltip("Curve to control light intensity throughout the day")]
		...
		[Header("Light Color"), Tooltip("Gradient to control light color throughout the day")]
		...
		private readonly string[] timeLabels =
		{
...
/*
		...
		private void Reset()
		{
			...
			for (var i = 0; i < intensityCurve.keys.Length; i++)
			{
...
*/
		...
		private void UpdateDebugInfo()
		{
			...
			var labelIndex = Mathf.FloorToInt(timeOfDay * timeLabels.Length);
		...
		public float GetCurrentDayFraction() => timeOfDay;
	}
}