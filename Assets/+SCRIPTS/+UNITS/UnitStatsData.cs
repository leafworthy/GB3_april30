...
namespace __SCRIPTS
{
	...
	public class UnitStatsData
	{
		...
		private static bool IsYes(Dictionary<string, string> row, string key) =>
			row.TryGetValue(key, out var value) && value.Trim().Equals("YES", StringComparison.OrdinalIgnoreCase);

		private static void TryFloat(Dictionary<string, string> row, string key, out float result)
		{
			result = 0f;
