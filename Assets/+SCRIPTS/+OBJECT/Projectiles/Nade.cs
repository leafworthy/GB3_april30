...
namespace __SCRIPTS.Projectiles
{
	public class Nade : MonoBehaviour
	{
		...
		bool hasLaunched;
		Vector3 jumpVel;
		float rotationSpeed = 330;
		float currentRotationSpeed = 130;
		ICanAttack owner;
		...
		void Move()
		{
			...
			jumpObject.transform.localPosition += jumpVel;
			jumpVel.y -= Services.assetManager.Vars.Gravity.y * Time.fixedDeltaTime;
			if (jumpObject.transform.localPosition.y < 0) jumpObject.transform.localPosition = new Vector3(0, 1, 0);
		}
	}
}
