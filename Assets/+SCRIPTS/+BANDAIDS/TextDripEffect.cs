...
namespace __SCRIPTS
{
	...
	public class TextDripEffect : MonoBehaviour
	{
		...
		private GameObject dripParticlePrefab;
		...
		[Header("Particle Appearance"), SerializeField, Range(0.1f, 3f)]
		...
		private void Awake()
		{
			...
			if (parentCanvas == null) enabled = false;

			if (dripParticlePrefab == null) enabled = false;
		}
		...
		{
			if (useRandomEmission) return emissionRate * (1f + Random.Range(-randomEmissionVariance, randomEmissionVariance));
			return emissionRate;
		}
		...
		private System.Collections.IEnumerator SpawnParticleAtEndOfFrame()
		{
			...
			var textWidth = textMesh.preferredWidth;
			if (textWidth <= 0) yield break; // No text to drip from

			// Calculate spawn position along the baseline of the text
			var xPos = Random.Range(-textWidth / 2, textWidth / 2);
			var localSpawnPos = new Vector2(xPos, -textMesh.preferredHeight / 2 + verticalOffset); // Baseline of text with offset

			// Create the particle
			var particleObj = Instantiate(dripParticlePrefab);
			...
			var particleRect = particleObj.GetComponent<RectTransform>();
			particleRect.anchoredPosition = rectTransform.anchoredPosition + localSpawnPos;

			// Set the scale
			var scale = baseScale;
			if (useRandomScale) scale *= 1f - randomScaleVariance / 2 + Random.value * randomScaleVariance;
