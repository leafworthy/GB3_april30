...
namespace CarbonInput.Demo {
    public class Demo : MonoBehaviour {
        ...
        void OnGUI() {
            GUI.Window(0, new Rect(20, 20, 640, 340), MyWindow, "GamePad");
        ...
        private void UnityAxis(int minx, int miny) {
            ...
            var names = Input.GetJoystickNames().Where(n => !string.IsNullOrEmpty(n)).ToArray();
            x = minx;
            foreach(var name in names) {
                var content = new GUIContent(name);
	            var width = GUI.skin.label.CalcSize(content).x;
                GUI.Label(new Rect(x, 320, width, 20), content);
	            x += (int)width + 10;
            }
        }
    }
